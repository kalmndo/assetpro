...
export default function Menu({
  ...
  id: string;
  audit: RouterOutputs["daftarAset"]["get"]["audit"];
}) {
    ...
    <>
      <DropdownMenu>
        ...
        <DropdownMenuContent side="bottom" align="end">
          ...
          <DropdownMenuItem
            onSelect={() => setDialog({ data: { id }, open: "susut" })}
          >
            <TrendingDown size={18} className="mr-2" /> Penyusutan
          </DropdownMenuItem>
          <DropdownMenuItem
            onSelect={() => setDialog({ data: { id }, open: "audit" })}
          >
            <UserSearch size={18} className="mr-2" /> Audit
          </DropdownMenuItem>
        </DropdownMenuContent>
      </DropdownMenu>
      <DialogQr id={id} open={dialog.open === "qr"} onClose={onClose} />
      <DialogInformasi
        id={id}
        open={dialog.open === "info"}
        onClose={onClose}
      />
      <DialogPenyusutan
        id={id}
        open={dialog.open === "susut"}
        onClose={onClose}
      />
      <DialogAudit
        data={audit}
        open={dialog.open === "audit"}
        onClose={onClose}
      />
    </>
  );
