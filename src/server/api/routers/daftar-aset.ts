...
export const daftarAsetRouter = createTRPCRouter({
  getAll: protectedProcedure.query(async ({ ctx }) => {
    ...
    const data = result.map((v) => ({
      ...
      masaManfaat: v.umur,
      harga: `Rp ${v.hargaPembelian.toLocaleString("id-ID")}`,
      nilaiSisa: "Rp 0",
      susut: `Rp ${v.nilaiPenyusutan.toLocaleString("id-ID")}`,
      hargaNum: v.hargaPembelian,
      susutNum: v.nilaiPenyusutan,
      nilaiBukuNum: v.nilaiBuku,
      nilaiBuku: `Rp ${v.nilaiBuku.toLocaleString("id-ID")}`,
      lokasi: v.Ruang?.name,
